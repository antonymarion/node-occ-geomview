...
THREE.LegacyJSONLoader_old = (function () {

    function LegacyJSONLoader_old(manager) {
    ...
    Object.assign(LegacyJSONLoader_old.prototype, {
...
class LegacyJSONLoader {
    ...
    parse(json) {
        let promise = new Promise((resolve, reject) => {
            try {
                //console.log("parse three JSON", json);

                // TODO: convert to GLTF
                const loader = new THREE.LegacyJSONLoader_old();
                var {geometry, materials} = loader.parse(json, null);
...
function GeomView(container, width, height) {
    ...
    // me.cameraControls = new CameraControls(me.camera, me.renderer.domElement);
    // me.cameraControls.dampingFactor = 0.001;

    var canvas2D = document.createElement("canvas");

    canvas2D.width = window.innerWidth;
    canvas2D.height = window.innerHeight;
    // me.cameraControls.setViewport(0, 0, canvas2D.width, canvas2D.height);
...
GeomView.prototype.zoomObject = function (node) {
    ...
    const me = this;
    const bbox = me.getObjectBox(node);
    if (bbox.isEmpty() || (me.selected && me.selected.constructor.name === "LabeledGrid")) {
        return;
    }
    const COG = bbox.getCenter();
